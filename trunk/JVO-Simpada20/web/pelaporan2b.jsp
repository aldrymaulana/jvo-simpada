				...
				<td colspan="3">
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						<tr class="JUDUL2" align="center" valign="middle">
							<td>No Urut</td>
							<td>No BKU Penerimaan</td>
							<td>No Rekening</td>
							<td>Jml Anggaran (Rp)</td>
							<td>Tgl Setor</td>
							<td>No STS dan<br>Bukti Penerimaan Lainnya</td>
							<td>Jumlah (Rp)</td>
						</tr>
						<%
							...
							if (request.getSession().getAttribute("htBKP") != null) {
								Hashtable htBKP = (Hashtable) request.getSession().getAttribute("htBKP");
								int inthtBKP = htBKP.size();
								int rowNum = 1;
								for (int a=1; a<=inthtBKP; a++) {
									String[] strhtBKP = (String[]) htBKP.get(String.valueOf(a));
									int c = rowNum % 2;
									String strNmClass;
									if (c == 0) {
										strNmClass = "TANYA";
									...
									%>
										<tr class="<%= strNmClass %>" align="center">
											...
											<td>&nbsp;</td>
											<td align="center"><%= strhtBKP[0] %></td>
											<td align="right"><%= new java.text.DecimalFormat("#,###,###,###,###.##").format(Double.parseDouble(strhtBKP[4])) %></td>
											<td align="center"><%= new java.text.SimpleDateFormat("dd/MM/yyyy").format(new java.text.SimpleDateFormat("yyyy-MM-dd").parse(strhtBKP[2])) %></td>
											<td>&nbsp;</td>
											<td align="right"><%= new java.text.DecimalFormat("#,###,###,###,###.##").format(Double.parseDouble(strhtBKP[3])) %></td>
										</tr>
									<%
									dblTotalPajak = dblTotalPajak + Double.parseDouble(strhtBKP[4]);
									dblTotalRetribusi = dblTotalRetribusi + Double.parseDouble(strhtBKP[3]);
