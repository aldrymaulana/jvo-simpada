	...
	if (request.getSession().getAttribute("htRincian") != null) {
		...
		for (int a=1; a<=intHtRincian; a++) {
			...
			<tr class="<%= strNmClass %>">
				...
				<td align="right">
					<input type="hidden" name="txtKodeRek_<%= a %>" id="txtKodeRek_<%= a %>" value="<%= strArray[0] %>"></input>
					<input <%= strInputStyle %> type="text" name="txtAnggaran_<%= a %>" id="txtAnggaran_<%= a %>" value="<%= new java.text.DecimalFormat("#,###,###,###,###.##").format(Double.parseDouble(strJumlah)) %>"></input> 
				</td>
			</tr>
		...
		<tr>
			<td colspan="3" align="center">
				...
				<button type="button" name="btCetak" id="btCetak" onclick="fnCetak()">Cetak</button>
				 -->
				<input style="display: none;" type="text" name="txtUbah" id="txtUbah"></input>
			</td>
		</tr>
	</table>
	</form>
