...
import com.tech.master.common.jvCommonClass;
import com.tech.master.common.jvGeneralBusiness;
...
public class srvLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String strAddWidth = jvCommonClass.fnGetProperty("ADD_WIDTH");
        double dobWidth = dim.getWidth() + Double.parseDouble(strAddWidth);
        String strWidth = String.valueOf((int)dobWidth); //jvc.fnGetProperty("SCR_WIDTH");
        request.getSession().setAttribute("strWidth", strWidth);

        String strAddHeight = jvCommonClass.fnGetProperty("ADD_HEIGHT");
        double dobHeight = dim.getHeight() + Double.parseDouble(strAddHeight);
        ...
        switch (Integer.parseInt(strMode)) {
            case 0: // '\0'
                try {
                    ...
                    out.println("var winprops = 'width='+lVarWidth+',height='+lVarHeight+',top=0,left=0,status=yes,resizable=yes,toolbar=no,menubar=no,scrollbars=no,location=no,titlebar=no,hotkeys=no';");
