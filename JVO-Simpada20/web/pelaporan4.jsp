...
<%
    ...
    String strKetPajak = request.getSession().getAttribute("strKetPajak").toString();
    
    String strTglAwal = request.getSession().getAttribute("strTglAwal").toString();
    String strTglAkhir = request.getSession().getAttribute("strTglAkhir").toString();
    
    String strKodePajak = request.getSession().getAttribute("strKodePajak").toString();
...
<body onload="fnChangePejabat();fnGetAllElement();">
    ...
    <input type="hidden" name="hidNamaBendahara" id="hidNamaBendahara" value="<%= strNamaBendahara %>">
   	...
   	<input type="hidden" name="strNamaKabid" id="hidNamaKabid" value="<%= strNamaKabid %>">
   	<input type="hidden" name="hidNIPKabid" id="hidNIPKabid" value="<%= strNIPKabid %>">
   	<input type="hidden" name="hidKodePajak" id="hidKodePajak" value="<%= strKodePajak %>">
   	<input type="hidden" name="hidKetPajak" id="hidKetPajak" value="<%= strKetPajak %>">
				...
				<td colspan="3">
					<table width="100%" border="0" cellpadding="2" cellspacing="2">
						<tr class="JUDUL2" align="center" valign="middle">
							<td valign="middle" rowspan="2">Nama </td>
						...
						<tr class="JUDUL2" align="center" valign="middle">
							<td valign="middle">Tgl Awal</td>
							<td valign="middle">Tgl Akhir</td>
							<td valign="middle">Tgl SKPD</td>
							<td valign="middle">Tgl Lunas</td>
						</tr>
						<%
							if (request.getSession().getAttribute("htRkpPajak") != null) {
								Hashtable htRkpPajak = (Hashtable) request.getSession().getAttribute("htRkpPajak");
								int intHtRkpPajak = htRkpPajak.size();
								int rowNum = 1;
								for (int a=1; a<=intHtRkpPajak; a++) {
									String[] strHtRkpPajak = (String[]) htRkpPajak.get(String.valueOf(a));
									int c = rowNum % 2;
									String strNmClass;
									if (c == 0) {
										strNmClass = "TANYA";
									...
									%>
										<tr class="<%= strNmClass %>">
											<td align="left"><%= strHtRkpPajak[0] %> / <%= strHtRkpPajak[1] %></td>
											<td align="left"><%= strHtRkpPajak[2] %></td>
											...
											<td align="center"><%= new java.text.SimpleDateFormat("dd MMM yyyy").format(new java.text.SimpleDateFormat("yyyy-MM-dd").parse(strHtRkpPajak[9])) %></td>
											<td align="center"><%= (strHtRkpPajak[10].trim().length() > 0)?new java.text.SimpleDateFormat("dd MMM yyyy").format(new java.text.SimpleDateFormat("yyyy-MM-dd").parse(strHtRkpPajak[10])):"-" %></td>
										</tr>
									<%
									rowNum = rowNum + 1;
								}
